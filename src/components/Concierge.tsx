...
/**
 * Simple parser to render markdown-like features (bold, links, lists) in the AI Concierge chat bubble.
 */
function renderContent(text: string) {
  ...
  return parts.map((line, idx) => {
    ...
    // Helper to format bold text inside a segment
    const formatBold = (str: string): React.ReactNode[] => {
      const boldParts = str.split(/\*\*([^*]+)\*\*/g);
      return boldParts.map((part, i) => {
        if (i % 2 === 1) {
          return <strong key={i} className="font-bold text-primary dark:text-teal-400">{part}</strong>;
        }
        return part;
      });
    };
    ...
    while ((match = linkRegex.exec(element)) !== null) {
      const textBefore = element.substring(lastIndex, match.index);
      renderedLine.push(...formatBold(textBefore));
      ...
      renderedLine.push(
        <a 
          key={match.index} 
          href={linkUrl} 
          className="text-primary dark:text-teal-400 hover:underline font-bold" 
    ...
    const remainingText = element.substring(lastIndex);
    renderedLine.push(...formatBold(remainingText));
    
    if (isListItem) {
      return (
        <div key={idx} className="flex gap-2 ml-2 my-1 items-start">
    ...
    return (
      <div key={idx} className={cn("min-h-[1.2em]", idx > 0 ? "mt-1.5" : "")}>
        {renderedLine}
      </div>
    );
  });
...
export function Concierge() {
  ...
  const suggestions = ["SSC Results", "Current Jobs", "UPSC Calendar", "State Exams"];
